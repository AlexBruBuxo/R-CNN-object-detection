"""
This is a test script that implements a region proposal object detection with OpenCV, Keras, and TensorFlow.
To perform this, it takes an image as an input, it splits the images into regions 
through a region proposal algorithm (i.e., selective search), these regions are then passed
through a classifier CNN Network (i.e., ResNet50), and a NMS (non-maxima supressor) algorithm
is used to reduce the overlaying ROIs with less probability. 

Source: https://pyimagesearch.com/2020/07/06/region-proposal-object-detection-with-opencv-keras-and-tensorflow/   

This file can be executed as follows:
$ python v3_region_proposal_detection.py --image images/bear.jpg (--filter bear)
"""
...
def selective_search(image, method="fast"):
    ...
    # otherwise we are using the *quality* but more slower version
    else:
...
# run selective search on the inout image
...
proposals = []  # store ROIs for ResNet classifier
boxes = []  # list of box coordinates (only sufficiently large regions will be included)
...
for (x, y, w, h) in rects:
    ...
    # extract the ROIs from the input image, convert it from BGR to
    ...
    # further preprocessing by the ROI
...
# loop over the predictions to filter the results based on --filter
# and --conf
for (i, p) in enumerate(preds):
    # grab the prediction information for the current region proposal
    (imagenetID, label, prob) = p[0]
    
    # only if the label filters are not empy *and* the label does not
    ...
    if prob >= args["conf"]:
        ...
        # bounding box + probability tot the list
        L = labels.get(label, [])
        L.append((box, prob))
        labels[label] = L
        # this results in a dictionary with the format as follows:
        # labels = {'house': [(box1, prob1), (box2, prob2), 'bear': [(box3, prob3)]]}

clone = image.copy()
clone_ = image.copy()
        
# loop over the labels for each of detected objects in the image,
# display the image with all bounding boxes, apply NMS, and display
# it again with final bounding boxes
for label in labels.keys():
    ...
    for (box, prob) in labels[label]:
        ...
        cv2.rectangle(clone_, (startX, startY), (endX, endY), 
                      [0, 255, 0], 2)
    
    # show the results *before* applying non-maxima suppression, then
    # clone the image again so we can display the results *after*
    # applying non-maxima suppression
    cv2.imwrite("./images/results/v3_before.jpg", clone_)
    ...
    # NOTE: NMS is applied to each label array independently!
    ...
    cv2.imwrite("./images/results/v3_after.jpg", clone)
    cv2.waitKey(0)
